...
#+begin_answer
1. \(\varphi: \mathbf{R} \rightarrow \mathbf{R}\), \(\varphi(x) = x\)
#+end_answer

* 3.F.3
  [2017-04-27 Thu 22:47]
...
1. 证明：\(v_{1},\ldots ,v_{m}\)张成\(V\)当且仅当\(\Gamma\)是单的。
2. 证明\(v_{1},\ldots ,v_{m}\)线性无关当且仅当\(\Gamma\)是满的。
#+end_problem
...
设\(v_{1},\ldots ,v_{m}\)是\(V\)的基，\(w_{1},\ldots ,w_{m}\in W\)，则存在唯一一个线性映射\(T:V\rightarrow W\)使得对于任意\(j=1,\ldots ,n\)都有：\[Tv_{j}= w_{j}\]这个定理的证明见 [[file:linear-map-on-vector-space.org][线性映射]] 一文。这个定理说明线性映射可以根据其在一个基上的取值来构造，而唯一性表明一个线性映射完全由其在基上的取值确定。时至今日，我对3.5的这个定理有了更深的理解。
...
#+end_answer
* 3.F.8
  [2017-04-29 Sat 17:27]

#+begin_problem
设\(m\)是正整数。

1. 证明\(1,x-5,\ldots ,(x-5)^{m}\)是\(\mathcal{P}_{m}(\mathbf{R})\)
2. 求1中的基的对偶基。
#+end_problem
...
1. 描述线性泛函\(T^{'}(\varphi_{1}),T^{'}(\varphi_{2})\)
2. 将\(T^{'}(\varphi_{1}),T^{'}(\varphi_{2})\)写成\(\psi_{1},\psi_{2},\psi_{3}\)的线性组合。
#+end_problem

#+begin_answer
1. 首先根据对偶映射的定义\(T^{'}(\varphi_{1}) = \varphi_{1}\circ T\) 所以：
\begin{equation}
\label{eq:5}
...
2. 对于\(v=(x,y,z)\in \mathbf{R}^{3}\)，因为\(\psi_{1},\psi_{2},\psi_{3}\)是\(\mathbf{R}^{3}\)的对偶基，则\(\psi_{1}(v) = x, \psi_{2}(v) = y,\psi_{3} (v) = z\)，显然：
\begin{eqnarray}
\label{eq:6}
