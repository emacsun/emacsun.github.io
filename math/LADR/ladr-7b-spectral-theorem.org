...
#+HTML_HEAD_EXTRA: <a id="pdf"  href="./ladr-7b-spectral-theorem.pdf"><img src="../../img/assets/pdf.png"></a>
...
1. \(U^{\bot}\)在\(T\)下不变。
2. \(T|_{U}\in \mathcal{L}(U)\)是自伴的。
3. \(T|_{u^{\bot}}\in \mathcal{L}(U^{\bot})\)是自伴的。
#+end_tikztheorem
#+begin_proof
...
#+begin_tikzinstance
考虑\(\mathbf{R}^{3}\)上的自伴算子\(T\)，其关于标准基的矩阵为：
\begin{equation}
\label{eq:14}
\begin{bmatrix}
14 & -13 & 8 \\
...
\begin{equation}
\label{eq:15}
\frac{(1,-1,0)}{\sqrt{2}},\qquad \frac{(1,1,1)}{\sqrt{3}},\qquad \frac{(1,1,-2)}{\sqrt{6}}
\end{equation}
是由\(\mathbf{R}^{3}\)的由\(T\)的本征向量构成的规范正交基，且\(T\)关于这个基的对角矩阵是：
\begin{equation}
\label{eq:16}
\begin{bmatrix}
27 & 0 & 0 \\
...
若\(\mathbf{F} = \mathbf{C}\),则复谱定理给出了\(V\)上正规算子的完全描述。由此可以完全描述\(V\)上的自伴算子。

若\(\mathbf{F} = \mathbf{R}\)，则实谱定理给出了\(V\)上自伴算子的完全描述。
