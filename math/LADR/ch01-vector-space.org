...
复数的算术性质：
1. 交换性(commutativity): 对所有的\(\alpha ,\beta \in \mathrm{C}\) 都有\(\alpha + \beta= \beta + \alpha,\alpha\beta = \beta\alpha\)
2. 结合性(associativity): 对所有的\(\alpha,\beta,\gamma \in \mathrm{C}\)都有 \((\alpha + \beta) + \gamma = \alpha + (\beta + \gamma), (\alpha\beta)\gamma = \alpha(\beta\gamma)\)
3. 单位元(identities): 对所有\(\gamma \in C\)都有 \(\gamma + 0 = \gamma,\gamma 1 =\gamma\)
4. 加法逆元(additive inverse): 对每个\(\alpha\in \mathrm{C}\)都存在唯一的\(\beta\)，使得\(\alpha + \beta = 0\)
5. 乘法逆元(multiplicative inverse)：对每个\(\alpha \in \mathrm{C}\)都存在唯一的\(\beta \in \mathrm{C}\)使得\(\alpha\beta = 1\)
6. 分配性质(distributive property)：对所有\(\gamma,\alpha,\beta\in \mathrm{C}\)都有\(\gamma(\alpha+\beta) = \gamma\alpha + \gamma\beta\)
...
* 向量空间
  [2017-03-08 Wed 22:29]

  向量空间就是带有加法和标量乘法的集合\(V\)，满足如下性质：
1. 交换性(commutativity): 对所有的\(u ,v \in \mathrm{V}\) 都有\(u + v= v + u\)
2. 结合性(associativity): 对所有的\(u,v,w \in \mathrm{V},a,b\in \mathbf{F}\)都有 \((u + v) + w = u + (v + w), (ab)v = a(bv)\)
3. 加法单位元(additive identity): 对所有\(w \in \mathbf{V}\)都有 \(0 + w =w\)
4. 加法逆元(additive inverse): 对每个\(u\in \mathrm{V}\)都存在唯一的\(v\)，使得\(u + v = 0\)
5. 乘法单位元(multiplicative identity)：对所有\(v\in V\)都有\(1v = v\)
6. 分配性质(distributive property)：对所有\(u,v\in V; a,b\in \mathbf{F}\)都有\(a(u+v) = au + av, (a+b)v = av + bv\)
...
1. 设\(S\)是一个集合，我们用\(\mathbf{F}^{S}\)表示从\(S\)到\(\mathbf{F}\)上所有函数的集合。
2. 对于\(f,g\in \mathbf{F}^{S}\),规定 \(f+g\in\mathbf{F}^{S}\)是如下函数：对所有\(x\in S\), \[(f+g)(x)=f(x)+g(x)\]
3. 对于\(\lambda\in \mathbf{F}\)和\(f\in \mathbf{F}^{S}\)规定\(\lambda f\in \mathbf{F}^{S}\)是如下函数：对所有的\(x\in S\)，\[(\lambda f)(x)=\lambda f(x)\]

举个例子：如果\(S\)是区间\([0,1]\),且\(\mathbf{F}=\mathbf{R}\)，则\(\mathbf{R}^{[0,1]}\)是定义在区间\([0,1]\)上的所有实值函数集合。


\(\mathbf{F}^{S}\)是向量空间，且满足如下三条性质：
1. 若\(S\)是非空集合，则\(\mathbf{F}^{S}\)是\(\mathbf{F}\)上向量空间。
2. \(\mathbf{F}^{S}\)的加法单位元是定义如下的函数\(0:S\rightarrow \mathbf{F}:\)，对所有\(x\in S\)有\[0(x) = 0\]
3. 对于\(f\in \mathbf{F}^{S}\)，\(f\)的加法逆元是如下定义的函数\(-f:S\rightarrow \mathbf{F}:\) 对所有\(x\in S\)，\[(-f)(x)=-f(x)\]
* 子空间
  [2017-03-11 Sat 09:47]
...
接下来我们看看几个子空间的例子：
#+begin_instance
a. 若\(b\in\mathbf{F}\) 则\({(x_{1},x_{2},x_{3},x_{4})\in \mathbf{F}^{4}:x_{3}=5x_{4}+b}\)是\(\mathbf{F}^{4}\)的子空间当且仅当\(b=0\);
...
* 子空间的直和
  [2017-03-11 Sat 10:55]

#+begin_definition
  设\(U_{1},\ldots,U_{m}\)都是\(V\)的子空间:
1. 和\(U_{1} + \ldots + U_{m}\)称为直和，如果\(U_{1},\ldots,U_{m}\)中的每个元素都可以唯一的表示成\(u_{1}+\ldots +u_{m}\),其中\(u_{j} \in U_{j}\)
2. 若\(U_{1} + \ldots + U_{m}\)是直和，则用\(U_{1} \oplus \ldots \oplus U_{m}\)来表示\(U_{1}+\ldots + U_{m}\)
#+end_definition
