...
2. \((\frac{1}{\sqrt{3}},\frac{1}{\sqrt{3}} ,\frac{1}{\sqrt{3}})\) \((-\frac{1}{\sqrt{2}},-\frac{1}{\sqrt{2}} ,0)\) 是\(\mathbf{F}^{3}\)的规范正交组
3. \((\frac{1}{\sqrt{3}},\frac{1}{\sqrt{3}} ,\frac{1}{\sqrt{3}})\) \((-\frac{1}{\sqrt{2}},-\frac{1}{\sqrt{2}} ,0)\), \((\frac{1}{\sqrt{6}}, \frac{1}{\sqrt{6}}, -\frac{2}{\sqrt{6}}) \) 是\(\mathbf{F}^{3}\)的规范正交组.
#+end_instance
...
#+begin_theorem
*舒尔定理* 设\(V\)是有限维的复向量空间且\(T\in \mathcal{L}(V)\)，则\(T\)关于\(V\)的某个规范正交基具有上三角矩阵。
#+end_theorem

* 内积空间上的线性泛函
  [2017-05-14 Sun 15:47]
...
#+begin_theorem
*里斯表示定理* 设\(V\)是有限维的且\(\varphi\)是\(V\)上的线性泛函，则存在唯一的向量\(u\in V\)使得对每个\(v\in V\)均有\(\varphi(v)= \langle v,u \rangle  \)
#+end_theorem
...
*解* 设\(\varphi(p) = \int_{-1}^{1} p(t)\cos(\pi t)dt\)，则：
\begin{eqnarray}
\label{eq:16}
