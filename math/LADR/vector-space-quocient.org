...
#+HTML_HEAD_EXTRA: <a id="pdf"  href="./vector-space-quocient.pdf"><img src="../../img/assets/pdf.png"></a>
...
#+begin_instance
1. 若\(U = \{(x,2x)\in \mathbf{R}^{2}:x\in \mathbf{R}\}\)，则\( \mathbf{R}^{2}/U\)是\( \mathbf{R}^{2}\)中所有斜率为\(2\)的直线的集合。
2. 若\(U\)是\(\mathbf{R}^{3}\)中包含远点的直线，则\(\mathbf{R}^{3}/U\)是所有平行于\(U\)的直线的集合。
3. 若\(U\)是\(\mathbf{R}^{3}\)中包含远点的平面，则\( \mathbf{R}^{3}/U\) 是 \(\mathbf{R}^{3}\)中所有平行于\(U\)的平面的集合。
