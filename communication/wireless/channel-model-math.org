...
#+HTML_HEAD_EXTRA: <a id="pdf"  href="./channel-model-math.pdf"><img src="../../img/assets/pdf.png"></a>
...
\end{eqnarray}
令 $\pmb{U}_{DFT} = \pmb{F}\otimes \pmb{I}_{N_R}) $ ， $\pmb{U}_{DFT}^{-1}= \pmb{F}^{-1}\otimes \pmb{I}_{N_T}) $ ，则它们都是酉矩阵。 $\pmb{F}$ 为傅里叶变换矩阵，记 $W_{K}^{kl} = e^{-j2\pi kl/K}$ ，则 $\pmb{F}$ 可以标示如下：
\begin{equation}
  \label{eq:20120319f}
  \pmb{F} =
  \begin{bmatrix}
