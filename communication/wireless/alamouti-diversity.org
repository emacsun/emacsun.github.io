...
#+HTML_HEAD_EXTRA: <a id="pdf"  href="./alamouti-diversity.pdf"><img src="../../img/assets/pdf.png"></a>
...
#+NAME:fig:mrrcarchitecture
[[../../img/communication_wireless/20171018alamouti1.png]]


传统的基于MRRC接收算法的接收分集系统框图如图([[fig:mrrcarchitecture]])。图中 $\mathbf{r}_0 = \mathbf{h}_0\mathbf{s}_0 + \mathbf{n}_0,\mathbf{r}_1 =\mathbf{h}_1\mathbf{s}_1 + \mathbf{n}_1$ ，其中 $\mathbf{s}_0$ 表示发射信号， $\mathbf{n}_0,\mathbf{n}_1$ 表示复高斯噪声信号。接收端的最大似然判决准则是：
\begin{equation}
  \label{eq:d2r0h0si}
...
\end{eqnarray}
把上式和~(\ref{eq:d2xy})带入~(\ref{eq:d2r0h0si})得到
\begin{equation}
...
\end{equation}
等幅度调制方式情况下，式~(\ref{eq:alpha02plusalpha12})可以简化为：
\begin{equation}
  \label{eq:d2s0si}
...
#+ATTR_HTML:  :width 600 :align center
#+NAME:fig:alamoutiarchitecture
[[../../img/communication_wireless/20171018alamouti1.png]]

Alamouti发送分集方案框图如图([[fig:alamoutiarchitecture]])。Alamouti分集方案可以分为三部分：1）发送端的编码，2）接收端合并，3）最大似然判决准则。

#+ATTR_HTML:  :width 600 :align center
[[../../img/communication_wireless/20171018alamouti3.png]]
...
合并方案：图([[fig:alamoutiarchitecture]])中的combiner生成以下两个信号：
\begin{eqnarray}
