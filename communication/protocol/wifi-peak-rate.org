...
#+KEYWORDS:
#+CATEGORIES:
#+SETUPFILE: ~/.emacs.d/org-templates/lte-level2.org
#+HTML_HEAD_EXTRA: <a id="pdf"  href="./wifi-peak-rate.pdf"><img src="../../img/assets/pdf.png"></a>
...
[[../../img/communication_protocol/20171018wifiPeakRate.png]]
...
#+ATTR_HTML:  :width 600 :align center
[[../../img/communication_protocol/20171018fft54.png]]

频域数据经过IFFT计算后转化到时域。在IEEE802.11a中，PPDU传输的时间--频域分布图，如下图所示。

#+ATTR_HTML:  :width 600 :align center
[[../../img/communication_protocol/20171018ppdu.png]]

由于在实际运算中，64个数据送入IFFT是按照上图进行映射的，但是协议在计算短训练符号和长训练符号过程中其频域数据都是以 $l_{-26,26}$ 或者 $L_{-26,26}$ 的形式给出的，也就是说计算IFFT时需要把标号为负的子载波放到IFFT输入数据标号较大的位置，这样在进行matlab仿真过程中就需要把短训练序列和长训练序列的频域形式进行翻转，这一过程可以使用ifftshift()或者fftshift()方便的实现。\\
...
#+ATTR_HTML:  :width 600 :align center
[[../../img/communication_protocol/2017101864fft.png]]
