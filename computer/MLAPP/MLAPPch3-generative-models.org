...
#+HTML_HEAD_EXTRA: <a id="pdf"  href="./MLAPP3-generative-models.pdf"><img src="../../img/assets/pdf.png"></a>
#+LaTeX_CLASS: cn-article

我们从判决理论导出生成模型（generative model）是机器学习中经常用到的一种模型。

* 判决理论
  [2017-06-08 Thu 21:32]
...
1. 首先解决inference问题找出\(p(x|C_{k})\)条件概率密度，然后找出\(p(C_{k})\),然后使用贝叶斯公式求出\(p(C_{k}|x)\)。这种模型叫做生成模型，之所以这么命名是因为这个模型对于以后的输入都可以产生比较好的输出。
2. 首先解决\(p(C_{k}|x)\)，然后使用判决理论将\(x\)赋值为\(C_{k}\)中的一个。这个模型叫做差分模型。
3. 找到一个差分函数\(f(x)\)把\(x\)映射为\(C_{k}\)中的一种。在这个模型中概率没有扮演任何角色。
